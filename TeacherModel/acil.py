...
model_path = {
    ...
    'sub_movie': 'saved/sub_movie.pth',
    'steam': 'saved/steam.pth',
}
dataset_name = sys.argv[1]
category2item = load_pickle(f'dataset/{dataset_name}/category.pickle')
config = Config(
    model="SASRec",
    ...
    if 'k' in batched_data:
        ...
        if candidate_item_list is None and target_category is not None:
            candidate_item_list = list(set(category2item[target_category[1:]]) - set(item_list)) \
                ...
                list(set(dataset.field2token_id['item_id'].keys()) - set(category2item[target_category[1:]]) - {'[PAD]'} - set(item_list))
        elif candidate_item_list is None:
            candidate_item_list = list(set(dataset.field2token_id['item_id'].keys()) - set(item_list) - {'[PAD]'})
        if candidate_item_list is not None:
            candidate_id_list = torch.tensor(
                [dataset.field2token_id['item_id'][_] for _ in candidate_item_list if _ in dataset.field2token_id["item_id"]], dtype=torch.int64).to(config["device"])

            score[candidate_id_list] += 1
            score -= 1
            score[..., dataset.field2token_id['item_id']['[PAD]']] = -torch.inf
        top_k_indices = torch.topk(score, batched_data['k']).indices
        top_k = [[dataset.field2id_token['item_id'][_] for _ in top_k_indices.tolist()]]
        ...
        candidate_id_lists = torch.tensor(
            [dataset.field2token_id['item_id'][_] for _ in candidate_item_list if _ in dataset.field2token_id["item_id"]],
...
def process_input(data):
    ...
    batched_data = {
        'user_id': torch.tensor(dataset.field2token_id['user_id'][users[0]], dtype=torch.int64, device=config["device"]),
        'item_length': torch.tensor(item_length[0], dtype=torch.int64, device=config["device"]),
        'item_id_list': torch.tensor([dataset.field2token_id['item_id'][_] for _ in item_lists[0]], dtype=torch.int64, device=config["device"]),
...
class ModelRunner:
    def __init__(self, model_name):
        ...
        self.model = get_model(config["model"])(config, train_data._dataset).to(config["device"])
        checkpoint = torch.load(model_path[dataset_name], map_location=config["device"])
    ...
    def run_model(self, batch_data):
        return self.model.full_sort_predict(batch_data).softmax(dim=1)
    ...
    async def model_runner(self):
        ...
        while True:
            ...
            batch_data = {
                ...
                'item_id_list': torch.stack([t["item_id_list"] for t in to_process], dim=0),
            }
            # print(batch_data)
    ...
    try:
        ...
        if data.get('immediately'):
            ...
            input_data = {
                ...
                'item_id_list': torch.unsqueeze(batched_data["item_id_list"], dim=0),
            }
...
# @app.route('/inference_i', methods=['POST'])
# async def inference(request):
#     try:
#         data = request.json
#         batched_data = process_input(data)
...
#             'item_id_list': torch.unsqueeze(batched_data["item_id_list"], dim=0),
#         }
#         scores = style_transfer_runner.run_model(batch_data)
#         batch_data['score'] = scores
#         process_output(batched_data)
#         top_k = batched_data['top_k']
#         return sanic.response.json({'inference': top_k}, status=200)
#     except HandlingError as e:
...
#
#
# @app.route('/ranking_i', methods=['POST'])
# async def ranking(request):
#     try:
#         data = request.json
#         batched_data = process_input(data)
...
#         batch_data['score'] = scores
#         process_output(batched_data)
#         _ranking = batched_data['_ranking']
#         return sanic.response.json({'ranking': _ranking}, status=200)
#     except HandlingError as e:
#         # we don't want these to be logged...
#         return sanic.response.text(e.handling_msg, status=e.handling_code)
...
if __name__ == '__main__':
    ...
    app.run(host="0.0.0.0", port=int(sys.argv[2]), debug=True, workers=int(sys.argv[4]))
